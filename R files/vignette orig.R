...
if(!preProc){
  ...
  data("Flu1918");
  # Incidence and generation timed distribution
  Iflu = Flu1918$incidence; genflu = Flu1918$si_distr
  ...
  data("SARS2003")
  # Incidence and generation timed distribution
  Isars = SARS2003$incidence; gensars = SARS2003$si_distr
  # Total infectiousness
  Lsars = overall_infectivity(Isars, gensars)
  
  # Start and end times for flu
  tflu = seq(2*win, length(Iflu)-(win-1)) + win-1
  # Start and end times for SARS
  tsars = seq(2*win, length(Isars)-(win-1)) + win-1
  ...
  tflu = read.csv('data/tflu.csv'); tflu = tflu[[1]]
  ...
  tsars = read.csv('data/tsars.csv'); tsars = tsars[[1]]
}
