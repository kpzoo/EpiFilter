% Renewal estimation with recursive Bayesian smoothers
clearvars; clc; close all; tic;

% Assumptions and notes
% - uses filtered predictions to be fair
...
saveTrue = 1; saveFol = join([saveFol num2str(scenNo)]);
...
% EpiFilter method
[~, ~, ~, ~, pR, pRup, pstate] = runEpiFilterSm(Rgrid, m, eta, nday, p0, Lam, Iday);
% EpiSmoother estimates for this trajectory
[Rmed, Rlow, Rhigh, Rmean, qR] = runEpiSmoother(Rgrid, m, nday, pR, pRup, pstate);

% EpiFilter one-step-ahead predictions 
[predF, predIntF] = recursPredict(Rgrid, pRup, Lam, Rmean);
...
hold on;
plotCIRaw(tplt', Iopt', Iconf(1, :)', Iconf(2, :)', 'b');
grid off; box off; hold off;
xlim([tplt(1) tplt(end)]);
...
scatter(tplt, Iplt, 'Marker', 'o', 'SizeData', 50, 'MarkerFaceColor', grey1,...
    'MarkerFaceAlpha', 0.9, 'MarkerEdgeAlpha', 0);
hold on;
plotCIRaw(tplt', IoptL', IconfL(1, :)', IconfL(2, :)', 'b');
grid off; box off; hold off;
xlim([tplt(1) tplt(end)]);
ylabel(['$\tilde{I}_{\tau(s)} | k = $ ' num2str(klong)], 'FontSize', 18)
...
ylabel(['$\tilde{I}_s | \eta = $ ' num2str(eta)], 'FontSize', 18)
xlabel('$s$ (days)', 'FontSize', 18);
linkaxes(ax1, 'xy'); linkaxes(ax2, 'xy');
if saveTrue
    cd(saveFol);
