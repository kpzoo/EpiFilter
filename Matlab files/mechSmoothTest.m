% Test mechanics of EpiSmoother but with varying grid and noise
...
for i = 1:lenm
    % Uniform prior and grid
    p0 = (1/m(i))*ones(1, m(i)); Rgridi = linspace(Rmin, Rmax, m(i));
    % EpiFilter estimates fed into smoother
    [~, ~, ~, ~, pR, pRup, pstate] = runEpiFilterSm(Rgridi, m(i), etam, nday, p0, Lam, Iday);
    [Rmedm{i}, Rlowm{i}, Rhighm{i}, Rmeanm{i}, qR] = runEpiSmoother(Rgridi, m(i), nday, pR, pRup, pstate);
end
...
% Uniform prior and grid
p0 = (1/me)*ones(1, me); Rgrid = linspace(Rmin, Rmax, me);
...
for i = 1:lene
    % EpiFilter estimates fed into smoother
    [~, ~, ~, ~, pR, pRup, pstate] = runEpiFilterSm(Rgrid, me, eta(i), nday, p0, Lam, Iday);
    [Rmede{i}, Rlowe{i}, Rhighe{i}, Rmeane{i}, qR] = runEpiSmoother(Rgrid, me, nday, pR, pRup, pstate);
end
...
ylabel(['$\hat{R}_{s} | \eta = $ ' num2str(etam)], 'FontSize', 18)
...
grid off; box off; hold off;
ylabel(['$\hat{R}_{s} | m = $ ' num2str(me)], 'FontSize', 18)
xlabel('$s$ (days)', 'FontSize', 18);
xlim([tday(5) tday(end)]); ylim([0 5]);
...
for j = 1:2:6
    ...
    grid off; box off; hold off;
    ylabel(['$\hat{R}_{s} | m = $ ' num2str(m(i))], 'FontSize', 18)
    if i == 3
...
for j = 2:2:6
    ...
    grid off; box off; hold off;
    ylabel(['$\hat{R}_{s} | \eta = $ ' num2str(eta(i))], 'FontSize', 18)
    if i == 3
